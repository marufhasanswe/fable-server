...
app.get("/api/books", async (req, res) => {
  try {
    const { userId, search, genre, sort } = req.query;

    const query = {};

    // writer specific books
    ...
    let cursor = ebooksCollection.find(query);
...
// Ebook delete api a
app.delete("/api/books/:id", verifyToken, writerVerify, async (req, res) => {
  const { id } = req.params;
